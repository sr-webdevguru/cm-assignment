...
!function () {
    "use strict";
    var e = {TAB: 9, ENTER: 13, ESC: 27, SPACE: 32, LEFT: 37, UP: 38, RIGHT: 39, DOWN: 40, SHIFT: 16, CTRL: 17, ALT: 18, PAGE_UP: 33, PAGE_DOWN: 34, HOME: 36, END: 35, BACKSPACE: 8, DELETE: 46, isControl: function (t) {
        var c = t.which;
        switch (c) {
            case e.SHIFT:
            case e.CTRL:
    ...
    void 0 === angular.element.prototype.querySelectorAll && (angular.element.prototype.querySelectorAll = function (e) {
        ...
        var c = this;
        c.parse = function (c) {
            var s = c.match(/^\s*(?:([\s\S]+?)\s+as\s+)?([\S]+?)\s+in\s+([\s\S]+?)(?:\s+track\s+by\s+([\s\S]+?))?\s*$/);
            if (!s)throw e("iexp", "Expected expression in form of '_item_ in _collection_[ track by _id_]' but got '{0}'.", c);
            return{itemName: s[2], source: t(s[3]), trackByExp: s[4], modelMapper: t(s[1] || s[2])}
    ...
    }]).controller("uiSelectCtrl", ["$scope", "$element", "$timeout", "RepeatParser", "uiSelectMinErr", function (t, c, s, l, i) {
        function n() {
        ...
        function r(t) {
            ...
            var s = o(h[0]), l = p.selected.length, i = 0, n = l - 1, a = p.activeMatchIndex, r = p.activeMatchIndex + 1, u = p.activeMatchIndex - 1, d = a;
            return s > 0 || p.search.length && t == e.RIGHT ? !1 : (p.close(), d = c(), p.activeMatchIndex = p.selected.length && d !== !1 ? Math.min(n, Math.max(i, d)) : -1, !0)
        }
        ...
        function u() {
            ...
            var s = t[p.activeIndex], l = s.offsetTop + s.clientHeight - e[0].scrollTop, n = e[0].offsetHeight;
            l > n ? e[0].scrollTop += l - n : l < s.clientHeight && (p.isGrouped && 0 === p.activeIndex ? e[0].scrollTop = 0 : e[0].scrollTop -= s.clientHeight - l)
        }

        var p = this, d = "";
        p.placeholder = void 0, p.search = d, p.activeIndex = 0, p.activeMatchIndex = -1, p.items = [], p.selected = void 0, p.open = !1, p.focus = !1, p.focusser = void 0, p.disabled = void 0, p.searchEnabled = void 0, p.resetSearchInput = void 0, p.refreshDelay = void 0, p.multiple = !1, p.isEmpty = function () {
        ...
        p.activate = function (e, t) {
            p.disabled || p.open || (t || n(), p.open = !0, p.activeMatchIndex = -1, p.activeIndex = p.activeIndex >= p.items.length ? 0 : p.activeIndex, s(function () {
            ...
            function s(e) {
                p.groups = [], angular.forEach(e, function (e) {
                    var s = t.$eval(c), l = angular.isFunction(s) ? s(e) : e[s], i = p.findGroupByName(l);
                    i ? i.items.push(e) : p.groups.push({name: l, items: [e]})
            ...
            var a = c ? s : n;
            p.parserResult = l.parse(e), p.isGrouped = !!c, p.itemProperty = p.parserResult.itemName, t.$watchCollection(p.parserResult.source, function (e) {
                ...
                if (e) {
                    var c = p.parserResult.source(t), s = c.filter(function (t) {
        ...
        p.refresh = function (e) {
            void 0 !== e && (v && s.cancel(v), v = s(function () {
                t.$eval(e)
            }, p.refreshDelay))
            ...
            var c = {};
            c[p.parserResult.itemName] = e, p.onSelectCallback(t, {$item: e, $model: p.parserResult.modelMapper(t, c)}), p.multiple ? (p.selected.push(e), p.sizeSearchInput()) : p.selected = e, p.close()
        }, p.close = function () {
            p.open && (n(), p.open = !1, s(function () {
                p.focusser[0].focus()
            }, 0, !1))
        }, p.removeChoice = function (e) {
            p.selected.splice(e, 1), p.activeMatchIndex = -1, p.sizeSearchInput()
        }, p.getPlaceholder = function () {
            return p.multiple && p.selected.length ? void 0 : p.placeholder
        }, p.sizeSearchInput = function () {
            var e = h[0], t = h.parent().parent()[0];
            h.css("width", "10px"), s(function () {
            ...
            t.$apply(function () {
                var t = !1;
                p.multiple && e.isHorizontalMovement(s) && (t = r(s)), !t && p.items.length > 0 && (t = a(s)), t && s != e.TAB && (c.preventDefault(), c.stopPropagation())
    ...
    }]).directive("uiSelect", ["$document", "uiSelectConfig", "uiSelectMinErr", "$compile", "$parse", function (t, c, s, l, i) {
        return{restrict: "EA", templateUrl: function (e, t) {
            var s = t.theme || c.theme;
            return s + (angular.isDefined(t.multiple) ? "/select-multiple.tpl.html" : "/select.tpl.html")
            ...
            var p = r[0], d = r[1];
            p.multiple = angular.isDefined(a.multiple), p.onSelectCallback = i(a.onSelect), d.$parsers.unshift(function (e) {
                var t, s = {};
                if (p.multiple) {
                    for (var l = [], i = e.length - 1; i >= 0; i--)s = {}, s[p.parserResult.itemName] = e[i], t = p.parserResult.modelMapper(c, s), l.unshift(t);
                    return l
                }
                return s = {}, s[p.parserResult.itemName] = e, t = p.parserResult.modelMapper(c, s)
            }), d.$formatters.unshift(function (e) {
                var t, s = p.parserResult.source(c, {$select: {search: ""}}), l = {};
                if (s) {
                    if (p.multiple) {
                        var i = [], n = function (e, s) {
                            if (e && e.length) {
                                for (var n = e.length - 1; n >= 0; n--)if (l[p.parserResult.itemName] = e[n], t = p.parserResult.modelMapper(c, l), t == s)return i.unshift(e[n]), !0;
                                return!1
                            }
                        };
                        if (!e)return i;
                        for (var a = e.length - 1; a >= 0; a--)n(p.selected, e[a]) || n(s, e[a]);
                        return i
                    }
                    var r = function (s) {
                        return l[p.parserResult.itemName] = s, t = p.parserResult.modelMapper(c, l), t == e
                    };
                    if (p.selected && r(p.selected))return p.selected;
                    for (var o = s.length - 1; o >= 0; o--)if (r(s[o]))return s[o]
                }
                return e
            }), p.ngModel = d;
            var h = angular.element("<input ng-disabled='$select.disabled' class='ui-select-focusser ui-select-offscreen' type='text' aria-haspopup='true' role='button' />");
            l(h)(c), p.focusser = h, p.multiple || (n.append(h), h.bind("focus", function () {
                ...
                return t.which === e.BACKSPACE ? (t.preventDefault(), t.stopPropagation(), p.select(void 0), void c.$digest()) : void(t.which === e.TAB || e.isControl(t) || e.isFunctionKey(t) || t.which === e.ESC || ((t.which == e.DOWN || t.which == e.UP || t.which == e.ENTER || t.which == e.SPACE) && (t.preventDefault(), t.stopPropagation(), p.activate()), c.$digest()))
            }), h.bind("keyup input", function (t) {
                t.which === e.TAB || e.isControl(t) || e.isFunctionKey(t) || t.which === e.ESC || t.which == e.ENTER || t.which === e.BACKSPACE || (p.activate(h.val()), h.val(""), c.$digest())
            ...
            }), p.multiple ? (c.$watchCollection("$select.selected", function (e) {
                d.$setViewValue(e, null, !0)
            }), h.prop("disabled", !0)) : c.$watch("$select.selected", function (e) {
                d.$viewValue !== e && d.$setViewValue(e)
            }), d.$render = function () {
                if (p.multiple && !angular.isArray(d.$viewValue)) {
                    if (!angular.isUndefined(d.$viewValue) && null !== d.$viewValue)throw s("multiarr", "Expected model value to be array but got '{0}'", d.$viewValue);
                    p.selected = []
                }
                ...
                if (c.removeAttr("ui-select-match"), 1 !== c.length)throw s("transcluded", "Expected 1 .ui-select-match but got '{0}'.", c.length);
                n.querySelectorAll(".ui-select-match").replaceWith(c);
                var l = t.querySelectorAll(".ui-select-choices");
                if (l.removeAttr("ui-select-choices"), 1 !== l.length)throw s("transcluded", "Expected 1 .ui-select-choices but got '{0}'.", l.length);
                n.querySelectorAll(".ui-select-choices").replaceWith(l)
            })
        }}
    }]).directive("uiSelectChoices", ["uiSelectConfig", "RepeatParser", "uiSelectMinErr", "$compile", function (e, t, c, s) {
        return{restrict: "EA", require: "^uiSelect", replace: !0, transclude: !0, templateUrl: function (t) {
            ...
            return function (l, i, n, a, r) {
                ...
                d.attr("uis-transclude-append", ""), s(i, r)(l), l.$watch("$select.search", function (e) {
        ...
        return{link: function (e, t, c, s, l) {
            l(e, function (e) {
                t.append(e)
            })
        }}
        ...
        return{restrict: "EA", require: "^uiSelect", replace: !0, transclude: !0, templateUrl: function (t) {
            var c = t.parent().attr("theme") || e.theme, s = t.parent().attr("multiple");
