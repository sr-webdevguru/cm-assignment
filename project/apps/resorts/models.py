...
from apps.routing.models import Domains
from apps.routing.utils import update_resort_discovery
from helper_functions import create_client, encrypt_patient_data_kms, encrypt_s3_files
...
class Country(models.Model):
    ...
    key = models.CharField(max_length=140, verbose_name="key for country")
...
class Location(models.Model):
    ...
    key = models.CharField(max_length=140, verbose_name="key id for location")
    country = models.ForeignKey(Country, verbose_name="country of location")
...
def get_upload_path(instance, filename):
    dir_name = os.path.join('static', str(instance.resort_id))
    return os.path.join(dir_name, str(uuid.uuid4()) + '.' +filename.split('.')[-1])


def validate_zoom_level(value):
    if value < 1 or value > 18 :  # Your conditions here
        raise ValidationError('zoom level must be between 1-18')
...
class UserResortMap(models.Model):
    user = models.ForeignKey(AUTH_USER_MODEL)
    resort = models.ForeignKey(Resort)
    role = models.ForeignKey('custom_user.UserRoles')
    user_status = models.IntegerField(choices=USER_STATUS, default=ACTIVE)
...
class Area(models.Model):
    area_pk = models.AutoField(primary_key=True, db_column="area_pk", verbose_name="area pk")
    area_id = models.UUIDField(db_column="area_id", verbose_name="area id", default=uuid.uuid4)
    resort = models.ForeignKey(Resort, db_column="resort", verbose_name="resort")
    area_name = models.CharField(db_column="area_name", verbose_name="area name", max_length=140, blank=False,
                                 null=False)
    area_status = models.IntegerField(db_column="area_status", verbose_name="area status", default=LIVE,
                                      choices=AREA_STATUS)


class ResortLocation(models.Model):
    location_pk = models.AutoField(primary_key=True, db_column="location_pk", verbose_name="location pk")
    location_id = models.UUIDField(db_column="location_id", verbose_name="location id", default=uuid.uuid4)
    area = models.ForeignKey(Area, db_column="area", verbose_name="area")
    location_name = models.CharField(db_column="location_name", verbose_name="location name", max_length=140,
                                     blank=False, null=False)
    map_lat = models.FloatField(db_column="map_lat", verbose_name="map lat", blank=False, null=False)
    map_long = models.FloatField(db_column="map_long", verbose_name="map long", blank=False, null=False)
    location_status = models.IntegerField(db_column="location_status", verbose_name="location status",
                                          choices=LOCATION_STATUS, default=LIVE)
    resort = models.ForeignKey(Resort, db_column="resort", verbose_name="resort")
